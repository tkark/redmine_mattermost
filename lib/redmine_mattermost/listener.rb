...
class MattermostListener < Redmine::Hook::Listener
	def redmine_mattermost_issues_new_after_save(context={})
		...
		channels.concat(channels_for_user(issue.assigned_to))
		channels.flatten!
	...
	def redmine_mattermost_issues_edit_after_save(context={})
		...
		channels = channels_for_project issue.project
		channels.concat(channels_for_user(issue.assigned_to,journal))
		channels.flatten!
		...
		attachment[:fields] = journal.details.map { |d| detail_to_field(d,journal) }
...
private
	...
	def channels_for_user(assigned_to,journal = nil)
		cf = UserCustomField.find_by_name("Mattermost Channel")

		users = Array.new
		if assigned_to && assigned_to.is_a?(Group)
			users << assigned_to.users
		elsif assigned_to
			users << assigned_to
		end

		# 元の担当者にも変更を通知する
		users << User.find(journal.detail_for_attribute('assigned_to_id').old_value) rescue nil
		users << Group.find(journal.detail_for_attribute('assigned_to_id').old_value).users rescue nil
		...
		users.each do |user|
			...
			channels << user.custom_value_for(cf).value if user.custom_value_for(cf).value.present? rescue nil
		end
		channels.uniq!

		# Channel name '-' or empty '' is reserved for NOT notifying
		return [] if channels.to_s == ''
		return [] if channels.to_s == '-'
		return channels.split(",") if channels.is_a? String
		channels
	end

	def detail_to_field(detail, journal = nil)
		field_format = nil
		...
		when "assigned_to"
			user = User.find(detail.value).to_s rescue nil
			pre_user = User.find(journal.detail_for_attribute('assigned_to_id').old_value).to_s if journal rescue nil
			user = Group.find(detail.value).to_s rescue nil unless user
			pre_user = Group.find(journal.detail_for_attribute('assigned_to_id').old_value).to_s if journal unless pre_user rescue nil

			user = l(:not_assigned) unless user
			pre_user = l(:not_assigned) unless pre_user

			value = escape pre_user.to_s+" "+l(:change_to)+" "+user.to_s
		when "fixed_version"
